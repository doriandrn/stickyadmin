...
class StickyStats extends StickyAdmin
{
    ...
    public static function init()
    {
        ...
        if ( StickyAdmin::$config['statistics']['ignore_spam'] ) {
        ...
        add_action('sticky_stats_purge', array(__CLASS__, 'sticky_stats_purge'));


        // Include and initialize vendor libraries
        if ( ! class_exists('Browser') && !class_exists('UserAgent') && !class_exists('BrowserDetection')) require_once STICKY_CLASSES_URI . '/vendor/browser-os/Browser.php';
        if ( ! class_exists('Mobile_Detect')) require_once STICKY_CLASSES_URI . '/vendor/mobile-detect/Mobile_Detect.php';
        require_once STICKY_CLASSES_URI . '/vendor/search-terms/SearchEngines.php';
        require_once STICKY_CLASSES_URI . '/vendor/search-terms/Countries.php';
        require_once STICKY_CLASSES_URI . '/vendor/geolite/Db/Reader.php';
        ...
        if ( ! isset( $_POST[ '_nonce' ] ) ) {
            update_option('sticky_stats_internal', get_option('sticky_stats_internal', 0) + 1);
        }

        // Update the options before shutting down
        // add_action('shutdown', array(__CLASS__, 'sticky_stats_save_options'));
    }
    ...
    public static function remove_spam($new_status = '', $old_status = '', $comment = '')
    {
        if ($new_status == 'spam' && !empty($comment->comment_author) && !empty($comment->comment_author_IP)) {
            self::$wpdb->query(self::$wpdb->prepare("DELETE FROM {$GLOBALS['wpdb']->prefix}sticky_stats_visits WHERE user = %s OR INET_NTOA(ip) = %s", $comment->comment_author, $comment->comment_author_IP));
        }
    }
    ...
    public static function new_blog($blog_id)
    {
        ...
        // self::$options = get_option('sticky_stats_options', array());
    }
    ...
    public static function sticky_stats_enqueue_tracking_script()
    {
        ...
        $params['ci'] = $encoded_ci . '.' . md5($encoded_ci);

        // if (self::$options['outbound_tracking'] == 'no') {
        //     $params['outbound_tracking'] = 'false';
        // }

        $params = apply_filters( 'sticky_stats_js_params', $params );

        wp_enqueue_script( 'sticky_stats' );
        wp_localize_script( 'sticky_stats', 'StickyStatsParams', $params );
    }

    /**
     * Remove old entries from the database
     */
    public static function sticky_stats_purge()
    {
        $purge_interval = intval( StickyAdmin::$config['statistics']['purge_interval'] );

        if ( $purge_interval <= 0 ) return;

        // Delete old entries
        $days_ago = strtotime(date_i18n('Y-m-d H:i:s')." -$purge_interval days");
        self::$wpdb->query("DELETE FROM {$GLOBALS['wpdb']->prefix}sticky_stats_visits WHERE `date` < $days_ago");

        // Optimize table
        self::$wpdb->query("OPTIMIZE TABLE {$GLOBALS['wpdb']->prefix}sticky_stats_visits");
    }
    ...
    public static function sticky_stats_track()
    {
        if ( StickyAdmin::$config['statistics']['tracker_active'] ) {
            ...
            exit(self::$data['id'].'.'.md5(self::$data['id']));
        }
    }
    ...
    public static function get_visitor_data()
    {
        ...
        // IP exclusion check
        if (!empty(self::$data['ip']) && !empty( StickyAdmin::$config['statistics']['ignore_ips'] ) ) {
            $ip_exclusions = explode("\n", StickyAdmin::$config['statistics']['ignore_ips'] );
        ...
        // Country exclusion
        if (!empty( StickyAdmin::$config['statistics']['ignore_countries'] ) && is_string( StickyAdmin::$config['statistics']['ignore_countries'] ) && stripos( StickyAdmin::$config['statistics']['ignore_countries'], self::$data['country']) !== false) {
            return false;
        }
        ...
        foreach (self::string_to_array( StickyAdmin::$config['statistics']['ignore_uas'] ) as $filter) {
        ...
        if (!empty($GLOBALS['current_user']->ID)) {
            ...
            if ( ! empty( StickyAdmin::$config['statistics']['ignore_users'] ) && is_string( StickyAdmin::$config['statistics']['ignore_users'] ) && strpos( StickyAdmin::$config['statistics']['ignore_users'], $GLOBALS['current_user']->data->user_login) !== false) {
                return false;
            }
        ...
        if (!empty(self::$data['resource'])) {
            foreach (self::string_to_array( StickyAdmin::$config['statistics']['ignore_pages'] ) as $filter) {
                $pattern = str_replace(array('\*', '\!'), array('(.*)', '.'), preg_quote($filter, '/'));
    ...
    protected static function get_insert_search_data()
    {
        if (!empty(self::$data['keywords'])) {
            ...
            $rows_updated = self::$wpdb->query(self::$wpdb->prepare("UPDATE ". $GLOBALS['wpdb']->prefix ."sticky_stats_searches SET `count` = count + 1 WHERE `terms` = '%s'", self::$data['keywords']));
    ...
    protected static function get_insert_referer_data()
    {
        if (!empty(self::$data['referer'])) {
            ...
            $rows_updated = self::$wpdb->query(self::$wpdb->prepare("UPDATE ". $GLOBALS['wpdb']->prefix ."sticky_stats_referers SET `count` = `count` + 1 WHERE `referer` = '%s'", self::$data['referer']));
    ...
    protected static function get_insert_post_data()
    {
        ...
        if (!empty(self::$data['resource']) && !empty(self::$data['resource']['content_id'])) {
            ...
            $rows_updated = self::$wpdb->query(self::$wpdb->prepare("UPDATE ". $GLOBALS['wpdb']->prefix ."sticky_stats_posts SET `hits` = `hits` + 1 WHERE `date` = '%s' AND  `post_id` = '%d'", self::$data['date'], self::$data['resource']['content_id']));
    ...
    {
        $rows_updated = self::$wpdb->query(self::$wpdb->prepare("UPDATE ". $GLOBALS['wpdb']->prefix ."sticky_stats_pageviews SET `hits` = `hits` + 1 WHERE `date` = '%s'", self::$data['date']));
    ...
    protected static function get_insert_os_data()
    {
        if (!empty(self::$data['os'])) {
            ...
            $rows_updated = self::$wpdb->query(self::$wpdb->prepare("UPDATE ". $GLOBALS['wpdb']->prefix ."sticky_stats_os SET `count` = count + 1 WHERE `os` = '%s'", self::$data['os']));
    ...
    protected static function get_insert_browser_data()
    {
        if (!empty(self::$data['browser'])) {
            ...
            $rows_updated = self::$wpdb->query(self::$wpdb->prepare("UPDATE ". $GLOBALS['wpdb']->prefix ."sticky_stats_browsers SET `count` = count + 1 WHERE `browser` = '%s'", self::$data['browser']));
    ...
    {
        if (isset($_COOKIE['sticky_stats_tracking_code'])) {
            list($identifier, $control_code) = explode('.', $_COOKIE['sticky_stats_tracking_code']);
    ...
    protected static function insert_data()
    {
        ...
        if ($visitor_cookie > 0) {
            ...
            self::insert_row(self::get_insert_data(), $GLOBALS['wpdb']->prefix . 'sticky_stats_visits');
            self::insert_row(self::get_insert_pageview_data(), $GLOBALS['wpdb']->prefix . 'sticky_stats_pageviews');

        } else {

            self::$data['id'] = self::insert_row(self::get_insert_data(), $GLOBALS['wpdb']->prefix . 'sticky_stats_visits', true);
            self::insert_row(self::get_insert_pageview_data(), $GLOBALS['wpdb']->prefix . 'sticky_stats_pageviews');
            self::insert_row(self::get_insert_search_data(), $GLOBALS['wpdb']->prefix . 'sticky_stats_searches');
            self::insert_row(self::get_insert_referer_data(), $GLOBALS['wpdb']->prefix . 'sticky_stats_referers');
            self::insert_row(self::get_insert_post_data(), $GLOBALS['wpdb']->prefix . 'sticky_stats_posts');
            self::insert_row(self::get_insert_os_data(), $GLOBALS['wpdb']->prefix . 'sticky_stats_os');
            self::insert_row(self::get_insert_browser_data(), $GLOBALS['wpdb']->prefix . 'sticky_stats_browsers');
    ...
    protected static function get_country()
    {
        ...
        $country = $reader->$country( self::get_ip() );
    ...
    protected static function parse_search_engine($referrerUrl) {
        ...
        global $StickyStats_SearchEngines, $StickyStats_SearchEngines_NameToUrl;
        ...
        $searchEngines = $StickyStats_SearchEngines;
        ...
        if (!$variableNames) {
            $searchEngineNames = $StickyStats_SearchEngines_NameToUrl;
    ...
    public static function getCountriesList($includeInternalCodes = false) {
        ...
        $countriesList = $GLOBALS['StickyStats_CountryList'];
        $extras = $GLOBALS['StickyStats_CountryList_Extras'];
