 ...
 */
  class BF_Admin_Page_Class {
    ...
    public function Load_page_hooker(){
      $page = $this->_Slug;
      //help tabs
      // add_action('admin-head', array($this,'admin_add_help_tab'));
      add_action('admin_head', array($this,'themes_creator_debug'));
      //pluploader code
          ...
          function load_images_muploader(){
            jQuery(".mupload_img_holder").each(function(i,v){
              if (jQuery(this).next().next().val() != ""){
                jQuery(this).append('<img src="' + jQuery(this).next().next().val() + '" style="width: 180px; height:auto" />');
        ...
        if (method_exists($this,'show_field_' . $field['type'])){
          ...
          switch($field['type']) {
            ...
            case 'OpenTab':
              ...
              break;
            case 'AccordionArea':
              echo '<div class="s_accordion" id="'.$field['id'].'">'."\n";
              break;
            case 'AccordionAEnd';
              echo '</div>';
              break;
            case 'h1':
              echo '<h1 class="s_page_title">'.$field['label'].'</h1>'."\n";
              break;
            case 'title':
            ...
            case 'subtitle':
              ...
              break;
            case 'paragraph':
              echo '<p class="desc">'.$field['text'].'</p>'."\n";
              break;
            case 'repeater':
     ...
     * @since 0.1
    */
    public function OpenDiv_Container() {
      $args['type'] = 'OpenDiv';
      $args['id']   = 'OpenDiv';
      $args['std']  = '';
      $this->SetField($args);
    ...
    */
     ...
     * 
    */
    public function AccordionArea($name) {
      $args['type'] = 'AccordionArea';
      $args['id']   = $name;
      $args['std']  = '';
      $this->SetField($args);
    }
    public function AccordionAEnd() {
      $args['type'] = 'AccordionAEnd';
      $args['id']   = '';
      $args['std']  = '';
      $this->SetField($args);
    }
     ...
     * @since 0.1
    */
    public function h1($label,$repeater = false) {
      $args['type']  = 'h1';
      $args['std']   = '';
      $args['label'] = $label;
      $args['id']    = 'title'.$label;
  ...
  public function load_scripts_styles() {
    ...
    wp_enqueue_style( 'Admin_Page_Class', $plugin_path . '/css/sticky_panel.css' );
  ...
  */
  ...
  public function check_field_time() {
    if ( $this->has_field( 'time' ) && $this->is_edit_page() ) {
      $plugin_path = $this->SelfPath;
      wp_enqueue_script( 'at-timepicker', $plugin_path . '/js/time-and-date/jquery-ui-timepicker-addon.js', array( 'jquery-ui-slider','jquery-ui-datepicker','jquery-ui-accordion' ), null, true );
    }
  }
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
   ...
   * @param string $meta 
   * @since 0.1
   * @author Doiran Tudorache
   * @access public 
   */
  public function show_field_themes( $field, $meta ) {
    if ( ! is_array( $meta ) )
      $meta = (array) $meta;
    $options = $field['options'];
      $this->show_field_begin( $field, $meta );
      echo '<div id="sticky_theme_selector">';
        echo '<ul>';
        $theme_counter = 0;
        
        foreach ( $options as $key => $value ) {
          $relval = strtolower(str_replace( ' ', '-', $value ));
          $theme_counter ++;
          echo "<li class='st sticky_theme_" . $theme_counter . " " . $relval. "'>";
            echo "<div class='theme_selector_inside'>";
              echo "<span class='theme_name'>" . $value . '</span>';
              echo "<span class='color_scheme'>";
                echo "<span></span>";
                echo "<span></span>";
                echo "<span></span>";
                echo "<span></span>";
  ...
  public function show_field_media_manager($field,$meta){
    ...
    $width = (isset($field['preview_width']))? $field['preview_width'] : '180px';
  ...
  public function save_field_themes(  $field, $old, $new ) {
  ...
  /**
   *  Sticky Themes Selector
   *  @author Dorian Tudorache
   *  @access public
   *  @param $options (array)  array of key => value pairs for select options  
   */
  public function add_s_ThemeSelector( $id, $args ) {
    $s_ts_type = array( 'type' => 'themes', 'name' => $args['name'], 'id' => $id, 'std' => '', 'options' => $args['options'] );
    $s_ts_type = array_merge( $s_ts_type, $args );
    $this->_fields[] = $s_ts_type;
  }
  /**
  ...
  public function show_import_export(){
    ...
    $ret ='
    <div class="apc_ie_panel form-table">
      <div class="apc_export field">
        ...
        <div class="desc-field">'. __('To export saved settings click the Export button bellow and you will get the export Code in the box bellow, which you can later use to import.','apc').'</div>
      </div>
      <div class="apc_import field">
        ...
        <div class="import_code">
         ...
         <div class="desc-field">'. __('To Import saved settings paste the Export output in to the Import Code box bellow and click Import.','apc').'</div>
      </div>
  ...
  public function export(){
    ...
    if ($options !== false) {
      $re['code']= "<!*!* STICKY Options [Export code] !*!*>\n".base64_encode(serialize($options))."\n<!*!* STICKY Options -- END !*!*>";
    } else {
   ...
   *
   */
  public function themes_creator_debug() {
    $options = get_option($this->option_group,false);
    // echo serialize($options);
  }
  public function add_s_ThemeLister( $themes ) {
    // foreach
  }
  /**
   * Ajax import 
  ...
  public function import(){
    ...
    $import_code = str_replace("<!*!* STICKY Options [Export code] !*!*>\n","",$import_code);
    $import_code = str_replace("\n<!*!* STICKY Options -- END !*!*>","",$import_code);
  ...
  public function download_file($content = null, $file_name = null){
    ...
    $content = "<!*!* STICKY Options [Export code] !*!*>\n".base64_encode(serialize($options))."\n<!*!* STICKY Options -- END !*!*>";
